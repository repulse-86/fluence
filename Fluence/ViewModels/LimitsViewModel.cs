...
namespace Fluence.ViewModels
{
    public class LimitsViewModel : INotifyPropertyChanged
    {
        ...
        public async Task LoadLimitsDetailsAsync()
        {
            if (IsBusy) return;
            IsBusy = true;
            try
            {
                var data = await _limitsService.GetLimitsAsync();
        ...
        public async Task<bool> SaveLimitsDetailsAsync()
        {
            ...
            try
            {
                ...
                await _limitsService.SaveLimitsAsync(data);
